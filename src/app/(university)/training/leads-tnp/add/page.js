...
export default function CollageDashboardPage() {
    ...
    // Fetch the Growth Manager users and count their leads (filtered by time range)
    async function fetchSalesReport() {
        try {
            // Query users with role "Growth Manager"
            ...
            // Fetch all leads from both collections
            ...
            // Filter leads based on createdAt date (assumed to be an ISO string)
            const filteredLeads = leadsDocs.filter((leadDoc) => {
                const leadData = leadDoc.data();
                const leadDate = new Date(leadData.createdAt);
                return leadDate >= startDate;
            });

            // Count filtered leads per manager (matching createdBy with manager.email)
            const counts = managers.map((manager) => {
                const count = filteredLeads.filter((leadDoc) => {
                    const leadData = leadDoc.data();
    ...
    // Re-run sales report fetch whenever the time range changes
    useEffect(() => {
        (async () => {
    ...
    // Initial fetch for counts (DataCards) and sales report
    useEffect(() => {
        (async () => {
    ...
    const handleTimeRangeChange = (range) => {
        setTimeRange(range);
        // The useEffect watching timeRange will refetch the sales report
    };
    ...
    // UniversityReport chart options and series
    const collegeChartOptions = {
        ...
        colors: ["#00E396"],
        xaxis: {
            categories: salesReportCategories, // Manager names
        },
        legend: {
            position: "bottom",
        },
    ...
    const collegeChartSeries = [
        {
            name: "Leads",
            type: "bar",
            data: salesReportSeries, // Lead counts for each manager
        },
    ];
