import TraineeLeadUpdateClient from "./TraineeLeadUpdateClient";
...
export default async function UpdateTNPLeadPage({params}) {
    ...
    if (docId) {
        try {
            ...
            const res = await fetch(`${domain}/api/training/leads-trainee/getById/${docId}`, {cache: "no-store"});
            if (res.ok) {
    ...
    return <TraineeLeadUpdateClient lead={oldData}/>;
}
